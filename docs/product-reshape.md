...
4. **Onboarding / time-to-value** — remove the setup gate; JIT profile; first-win flow. **✅ FIRST CUT BUILT** — the app already lands usable (goal/settings defaults exist, no hard gate beyond the opt-in PIN), so the gap was the *why*. Added a **first-run welcome card** to the dashboard (`first_run` = no transactions and no food logged, ever): states the value prop ("Track what you spend and what it does to your body") + two first-win buttons (Add your first expense / Log your first meal) + a "no lengthy setup, defaults you can tweak" line. Auto-hides the moment anything is logged. Verified hidden on a populated DB + compile; not seen live (needs a genuinely fresh install — didn't risk swapping the real DB). *Remaining slice:* just-in-time **goal estimation** (estimate calorie/protein targets from a couple of taps instead of the full profile form).
5. **Deepen the intersection** — cost-per-macro, eating-out money+health, the unified spend+meal log entry. **✅ FIRST CUT BUILT (cost-per-macro)** — `PantryItem` already carries both `price` and `protein_g`, so the dashboard now shows a **"Best value protein"** card ranking pantry items by protein-per-£ (best 3 + worst) — the signature "eat well on a budget" insight no single-purpose app produces. Shown only when ≥2 priced pantry items exist. Verified in-browser with temp data (Chicken 41g/£1 … Protein bars 7g/£1). *Remaining slices:* eating-out **calories** and the **unified spend+meal log** both need `FoodLog` to carry a cost / a link to the spend (a small data-model add) — deferred; the money-side eating-out nudge already ships on the dashboard (Phase 3).
6. **Package & deploy** — one-command self-host (e.g. Docker image), polish, README/setup. **✅ BUILT** — added `Dockerfile` (python:3.12-slim, deps-cached, copies backend/frontend/run.py, `BALANCE_HOST=0.0.0.0`, `VOLUME /app/data`, `EXPOSE 8000`), `docker-compose.yml` (`docker compose up -d` → :8000, `./data` bind-mount for transparent backups, `restart: unless-stopped`, `TZ`), `.dockerignore`, and `DEPLOY.md` (quick start, data/backup story, "don't point two instances at one ./data" warning, update flow, no-Docker fallback). **Docker itself isn't installed on the dev box**, so the image build wasn't run — but the container's actual runtime behaviour was verified by running `run.py` exactly as the container does: a throwaway copy on a fresh empty data dir + `BALANCE_PORT`/`BALANCE_HOST` env booted clean (200), created the DB, ran the lean backfill, and served the **fresh-install UX** (lean 5-item nav + first-run welcome) — which also finally validated Phases 2 & 4 live. **Image build + run now VERIFIED (6 Sep 2026):** `docker compose build` succeeded (all wheels, ~14s), and `docker compose up -d` against the real `./data` served the live DB (285 transactions, integrity check ok, dashboard rendered) before `docker compose down`. Note: the image pulls the latest NiceGUI (3.16.0) via the `>=` pins — pin exact versions in requirements.txt if you want the container to match the bare-metal venv (3.15.0).
