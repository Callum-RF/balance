...
2. **Declutter** — retier nav to Core; move money-only/health-only/power behind modules (off by default). Instant "focused app" feel.
3. **Actionable dashboard** — rebuild as verb-driven cards incl. a first cut of the crossover insights (the "why" shows up early).
